<template>
  <b-card no-body class="p-2 shadow-none border">
    <div class="no-print">
          ...
          <table class="table table-hover mb-0">
            <thead class="bg-light text-secondary small font-weight-bold">
              <tr>
                ...
                <th style="width: 10%">PO BATCH</th>
                <th style="width: 20%">CODE SAP</th>
                <th style="width: 25%">DESCRIPTION (รายการ)</th>
                <th style="width: 15%">COLOR (สี)</th>
      ...
      <div v-else class="bom-detail-area mt-4">
        ...
        <b-button 
          ...
          block
        >
          🖨️ พิมพ์ใบสั่งผลิตและตัดยอดคงเหลือ (A4 แนวนอน)
        </b-button>
    ...
    <div id="print-content" class="d-none">
      <div style="font-family: 'Sarabun', sans-serif; color: black; padding: 0;">
        <h1 style="text-align: center; font-weight: bold; margin-bottom: 10px; margin-top: 0; font-size: 24px;">ใบสั่งผลิตชิ้นส่วน (Production Order)</h1>
        <div style="display: flex; justify-content: space-between; border-bottom: 2px solid #ccc; padding-bottom: 8px; margin-bottom: 10px;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 15px;">
          <thead>
            <tr>
              <th style="border: 1px solid black; padding: 6px; text-align: center; background-color: #f2f2f2;">#</th>
              <th style="border: 1px solid black; padding: 6px; text-align: center; background-color: #f2f2f2;">รหัสชิ้นส่วน</th>
              <th style="border: 1px solid black; padding: 6px; text-align: left; background-color: #f2f2f2; width: 35%;">รายการชิ้นส่วน</th>
              <th style="border: 1px solid black; padding: 6px; text-align: center; background-color: #f2f2f2;">ต่อตัว</th>
              <th style="border: 1px solid black; padding: 6px; text-align: center; background-color: #f2f2f2;">ต้องใช้</th>
              <th style="border: 1px solid black; padding: 6px; text-align: center; background-color: #f2f2f2;">สต็อก</th>
              <th style="border: 1px solid black; padding: 6px; text-align: center; background-color: #f2f2f2;">ผลิตเพิ่ม</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(row, index) in tableData" :key="index">
              <td style="border: 1px solid black; padding: 4px; text-align: center;">{{ index + 1 }}</td>
              <td style="border: 1px solid black; padding: 4px; text-align: center; font-weight: bold;">{{ row.partCode }}</td>
              <td style="border: 1px solid black; padding: 4px; text-align: left;">{{ row.itemName }}</td>
              <td style="border: 1px solid black; padding: 4px; text-align: center;">{{ row.perUnit }}</td>
              <td style="border: 1px solid black; padding: 4px; text-align: center; font-weight: bold;">{{ row.totalNeed.toLocaleString() }}</td>
              <td style="border: 1px solid black; padding: 4px; text-align: center;">{{ row.stock.toLocaleString() }}</td>
              <td style="border: 1px solid black; padding: 4px; text-align: center; font-weight: bold; font-size: 1.1rem;">{{ row.toProduce.toLocaleString() }}</td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  methods: {
    ...
    updateCalculation() {
      ...
      // ส่วนที่แก้ใหม่: กรองรายการที่ไม่มีรหัส partCode (บรรทัดว่าง) ออกไปก่อนคำนวณ
      const validComponents = components.filter(item => item.code && item.code.trim() !== "");

      this.tableData = validComponents.map(item => {
